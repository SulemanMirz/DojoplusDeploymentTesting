...
const CheckInCard: React.FC<CheckInCardProps> = ({
  ...
  const classDuration =
    parseInt(checkInData?.classDurationFromClassId?.[0]) / 60;
  const date = new Date(checkInData?.createdTime).toLocaleString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
  });
      ...
      <CheckIn>
        <AvatarContainer>
          <Avatar
            variant="circular"
            src={
              checkInData?.schoolLogoFromSchoolLinkFromClassId?.[0]?.url ||
              defaultSchoolLogo
            }
          />
        </AvatarContainer>
        <CheckInWrapper>
          ...
          <ScheduleEventNameStyled>
            {checkInData?.classNameFromClassName?.[0]}
          </ScheduleEventNameStyled>
          <TextGray12Opacity style={{ fontWeight: 400 }}>
            {checkInData?.martialArtsLinkFromClassId?.[0]}
          </TextGray12Opacity>
        </CheckInWrapper>
      </CheckIn>
    </>
